...
/**
 *	\file       htdocs/core/modules/facture/doc/pdf_transact.modules.php
 *	\ingroup    facture
 *	\brief      File of class to generate customers invoices from transact model
 */
...
/**
 *	Class to manage PDF invoice template transact
 */
class pdf_transact extends ModelePDFFactures
{
    var $db;
    var $name;
    var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "transact";
		$this->description = "transact template for Invoicing";
	...
	function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		if ($object->type != 2)
		{
          ...
          $posy+=2;
			...
			if (empty($object->mode_reglement_code) || $object->mode_reglement_code == 'VIR')
			{
				if (! empty($object->fk_bank) || ! empty($conf->global->FACTURE_RIB_NUMBER))
				{
					...
					$account = new Account($this->db);
					$account->fetch(3); // transact payments
          ...
          // // Display special transact payments disclaimer
          // $posy+=3;
          // $pdf->SetXY($this->marge_gauche, $posy);
          // $pdf->MultiCell(100, 3,'We give you notice that Advance Bookkeeping Centre Pty Ltd ("The Assignor") has assigned to Transact Payments Pty Ltd absolutely the debt referred to in this Tax Invoice owing by you pursuant to this.',0,'L',0);

          // $posy=$pdf->GetY()+1;
          // $pdf->SetXY($this->marge_gauche, $posy);
          // $pdf->MultiCell(100, 3,'You are irrevocably directed to pay the debt to Transact Payments Pty Ltd.',0,'L',0);
   ...
   *  @return	void
   */
  function pdf_bank_custom(&$pdf,$outputlangs,$curx,$cury,$account,$onlynumber=0,$default_font_size=10)
  {
  	global $mysoc, $conf;
