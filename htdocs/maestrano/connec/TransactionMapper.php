...
class TransactionMapper extends BaseMapper {
  ...
  protected function mapConnecResourceToModel($transaction_hash, $transaction) {
    ...
    // TODO!!
    if($this->is_set($transaction_hash['code'])) { $transaction->column_fields['facnumber'] = $transaction_hash['code']; }
    if($this->is_set($transaction_hash['ref_ext'])) { $transaction->column_fields['transaction_number'] = $transaction_hash['ref_ext']; }
